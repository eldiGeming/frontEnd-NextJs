...
const TableMahasiswa = () => {
  ...
  const handleDelete = async (nim: string) => {
    try {
      ...
      setMahasiswaData(
        mahasiswaData.filter((mahasiswa: any) => mahasiswa.nim !== nim),
      );
    } catch (error: any) {
  ...
  return (
    <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
      ...
      <div className="flex flex-col">
        ...
        {/* Data tabel */}
        {mahasiswaData.map((mahasiswa: any, index) => (
          <div
            className={`grid grid-cols-6 sm:grid-cols-6 ${index === mahasiswa.length - 1 ? "" : "border-b border-stroke dark:border-dark-3"}`}
            key={mahasiswa.id}
          >
            ...
            <div className="flex items-center justify-center gap-2 px-2 py-4">
              <Link href={"/master/master-mahasiswa/?nim=" + mahasiswa.nim}>
